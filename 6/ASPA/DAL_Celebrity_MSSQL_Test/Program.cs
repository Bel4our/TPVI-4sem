...
internal class Program
{
    ...
    {
        string CS = @"Data Source=DESKTOP-P1GMMIV; Initial Catalog=LESO1; Integrated Security=True; TrustServerCertificate=True;";
        ...
        using (IRepository repo = Repository.Create(CS))
        {
            ...
            }
            {
                ...
                if ((id = repo.GetCelebrityIdByName("Huntington")) > 0)
                {
                    Lifeevent le = new Lifeevent { CelebrityId = id, Description = "New Life Event", Date = new DateTime(1927, 04, 18) };
            ...
            }
            {
                ...
                if ((id = repo.GetCelebrityIdByName("Huntington")) > 0)
                {
                    Lifeevent le = new Lifeevent { CelebrityId = id, Description = "New Life Event", Date = new DateTime(2008, 12, 24) };
                    if (repo.AddLifeevent(le)) Console.WriteLine($"OK: AddLifeevent: {printL(le)}");
                    else Console.WriteLine($"ERROR: AddLifeevent: {printL(le)}");
                }
                else Console.WriteLine($"ERROR: GetCelebrityIdByName");
